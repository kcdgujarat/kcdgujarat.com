...
_Last updated: 2026-08-12 (speaker photos uncropped + content-hashed; schedule aligned to the schedulelist export; track + hall filters)_
...
- Photos: `node scripts/import-speaker-photos.mjs "<speakers photo export>.zip"` (a folder works too) copies each file **byte for byte** to `public/images/speakers/<slug>-<hash8>.<ext>` — no resize, no re-encode, **no crop** (~21.5 MB for 33). The 8-char digest of the contents is the cache-buster; the script also rewrites the `photo:` line in each speaker markdown to the file it wrote, so the extension and hash can never drift out of sync. Re-running with the same export is a no-op. Unmatched filenames are a hard error; aliases in `SLUG_ALIASES`. `dhwani-suthar` (400×400) and `rudraksh-karpe` (460×460) are below the 512 minimum — ask for bigger files.
...
51. **Speaker photos are no longer cropped** — The importer used to force every photo into an 800×800 `fit: 'cover'` square with sharp's attention strategy, and `SpeakerCard` + the speaker detail page compounded it with `object-cover`. 19 of the 33 photos in the export aren't square (0.56 to 1.40), so most portraits lost part of the head. Photos now ship exactly as supplied and both frames use `object-contain`. Filenames carry a content hash because the first fix looked like it had failed: the bytes on disk were right, but next/image and the browser both cache by URL, so the old cropped square kept being served under the unchanged `<slug>.jpg`. Verified served ratios against the sources (raghu-reddy 0.56, atulpriya 0.71, amritansh 0.73, aman-mundra 1.13, anish-bista 1.40).
...
- **Cropping speaker photos to a square** — the original importer did this with sharp's attention crop, on the theory that a fixed square in Git beats letting `object-cover` guess. It cut the heads off most of the non-square headshots. Never crop a speaker's photo; letterbox it.
