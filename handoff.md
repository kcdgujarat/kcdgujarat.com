...
53. **Lightning-slot caption** — parallel rows whose talks are all `type: Lightning` (the 16:30 block onwards) now read "2 parallel lightning talks" instead of "2 parallel sessions". Driven off session type, so moving a lightning talk earlier flips the caption with it. Also filled in the missing `slotLabel` helper the grid was already calling.
